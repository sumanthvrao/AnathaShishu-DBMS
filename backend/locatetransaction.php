...
$query= pg_query($conn, "SELECT t.receipt_no,name,phno,email_id,donation_cause,amount FROM Donation_Money as d,transaction as t where t.receipt_no=d.receipt_id and dateis='$date'");
if(!$query)
...
$query= pg_query($conn, "SELECT t.receipt_no,name,phno,email_id,item,quantity FROM Donation_Kind as d,transaction as t where t.receipt_no=d.receipt_no and dateis='$date'");
if(!$query)
	echo "An error occured";
